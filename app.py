...
def load_shopify_seasonal_data(
    ...
    """Load live inventory + up to 30 days of sales for one Seasonal tag.

    This intentionally avoids Shopify Bulk Operations. A normal paginated Orders
    query is much faster for the short Seasonal lookback window. Product publication
    dates are fetched separately so products younger than 30 days use their actual
    selling age as the velocity denominator.
    """
    credentials = ShopifyCredentials(
        shop=shop,
        client_id=client_id,
    ...
    if not clean_tag or not product_ids:
        return (
            pd.DataFrame(),
            pd.DataFrame(columns=["date", "product_id", "variant_id", "quantity"]),
            {"selected_tag": clean_tag, "seasonal_products": 0, "seasonal_variants": 0},
        )

    inventory, inventory_meta = client.fetch_product_inventory(list(product_ids))
    if inventory.empty:
        return (
            inventory,
            pd.DataFrame(columns=["date", "product_id", "variant_id", "quantity"]),
            {
                **dict(inventory_meta or {}),
    ...
    # The Seasonal model is already scoped by the exact selected Shopify tag. Keep a
    # tags column so the dataframe matches the same schema used by the Repeat model.
    inventory = inventory.copy()
    inventory["tags"] = clean_tag

    # Publication date is the preferred launch date. createdAt is only a fallback for
    # products that have no publication timestamp.
    launch_rows: list[dict] = []
    product_query = """
    query MantraSeasonalProductDates($ids: [ID!]!) {
      nodes(ids: $ids) {
        ... on Product {
          id
          createdAt
    ...
    for batch_start in range(0, len(product_ids), 100):
        batch_ids = list(product_ids[batch_start : batch_start + 100])
        data = client.graphql(product_query, {"ids": batch_ids})
    ...
    launch_map: dict[str, object] = {}
    if launch_rows:
        launch_df = pd.DataFrame(launch_rows)
        launch_df["sales_start_date"] = launch_df["published_at"].fillna(launch_df["created_at"])
        launch_map = dict(zip(launch_df["product_id"], launch_df["sales_start_date"]))
    inventory["sales_start_date"] = inventory["product_id"].astype(str).map(launch_map)
    ...
    # Seasonal only needs the latest 30 calendar days. Shopify's Orders search can
    # filter by SKU, so when every selected variant has a SKU we query only orders
    # containing those SKUs instead of scanning every store order in the window.
    ...
    if all_variants_have_sku and target_skus:
        # Keep each Shopify search string modest in size. Results are de-duplicated by
        # order ID because one order can contain SKUs from more than one chunk.
        sku_chunks = [target_skus[i : i + 30] for i in range(0, len(target_skus), 30)]
        order_searches = [
            date_filter
            + " ("
        ...
        # Correctness fallback for any selected Shopify variant that has no SKU.
    ...
    for order_search in order_searches:
        ...
        while True:
            ...
            for order in connection.get("nodes") or []:
                ...
                line_after = (line_connection.get("pageInfo") or {}).get("endCursor")
                has_more_lines = bool((line_connection.get("pageInfo") or {}).get("hasNextPage"))

                # Normal Mantra orders should fit in the first 50 lines. Only unusually
                # large orders pay for additional line-item pagination.
                while has_more_lines:
                    ...
                    more_page = more_conn.get("pageInfo") or {}
                    has_more_lines = bool(more_page.get("hasNextPage"))
                    line_after = more_page.get("endCursor")
    ...
    meta = {
        **dict(inventory_meta or {}),
        ...
        "sales_method": "Shopify paginated orders (30-day Seasonal window)",
...
def _seasonal_classify_x(relative: float, tolerance: float) -> tuple[int, bool, str]:
    """Use the exact same 1X/2X/3X thresholds as the Repeat model."""
...
def build_seasonal_metrics(
    ...
    ago use only their actual observed selling days as the denominator. The season
    end date caps the target inventory horizon so the model does not intentionally
    buy coverage beyond the season.
    """
    if inventory.empty:
    ...
    for product_id, product_inv in inventory.groupby("product_id", sort=False):
            ...
            product_variant_rows.append(
                {
                    ...
                    "alert_status": _seasonal_alert_label(priority, variant_trigger, enabled),
        ...
        for row in product_variant_rows:
            if row["effective_ideal"] > 0 and not pd.isna(smallest_positive):
                relative = row["effective_ideal"] / smallest_positive
                x_weight, borderline, confidence = _seasonal_classify_x(relative, tolerance)
            else:
                relative = math.nan
                                ...
                                with st.expander(expander_title, expanded=True):
                                            ...
                                            order_x_values.append(
                                                int(
                                                    st.session_state.seasonal_variant_order_x_overrides.get(
                                                        state_key, int(variant_row["model_x"])
                                                    )
                                                )
                                            )
                                    editable["order_x"] = order_x_values
                                    ...
                                    for row_index, edit_row in edited.iterrows():
                                        ...
                                        new_order_x = (
                                            ...
                                            else max(0, int(raw_new_order_x))
                                        )
                                        if new_order_x != previous_order_x:
                                            model_x = int(editable.loc[row_index, "model_x"])
                                            if new_order_x == model_x:
