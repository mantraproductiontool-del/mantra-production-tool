...
def _configure_repeat_client(client: ShopifyClient) -> None:
    """Use robust Repeat discovery and temporarily disable Shopify incoming stock."""
    # ShopifyClient.fetch_repeat_inventory() calls these instance attributes.
    # Override them only for this client instance; the connector file itself is untouched.
...
def load_shopify_model_data(
    ...
    if "incoming_qty" in inventory_df.columns:
        inventory_df = inventory_df.copy()
        inventory_df["incoming_qty"] = 0
    meta = dict(meta or {})
    meta["incoming_method"] = "temporarily disabled; forced to 0"
...
def test_shopify_connection(
    ...
    inventory_meta["incoming_method"] = "temporarily disabled; forced to 0"
...
def load_shopify_seasonal_data(
    ...
    Variants + current inventory + incoming inventory + launch dates are fetched in
    compact ProductVariant pages. Recent orders are queried in conservative pages so
    ...
    # inventory-item batches. Incoming inventory is nested at a deliberately small
    # location page size. Mantra normally has far fewer than 10 locations; if an item
    # exceeds that, only that exceptional item receives follow-up pagination.
    variant_query = """
    ...
    """
    inventory_more_query = """
    query MantraSeasonalMoreInventory($id: ID!, $after: String) {
      inventoryItem(id: $id) {
        inventoryLevels(first: 25, after: $after) {
          pageInfo { hasNextPage endCursor }
          nodes {
            location { id name }
            quantities(names: ["incoming"]) { name quantity }
    ...
    def _incoming_from_levels(levels: list[dict]) -> tuple[int, set[str]]:
        incoming = 0
        ...
        for level in levels:
            ...
            for quantity in level.get("quantities") or []:
                if str(quantity.get("name") or "") == "incoming":
                    incoming += int(quantity.get("quantity") or 0)
        return incoming, locations
    ...
    for product_start in range(0, len(product_ids), 40):
        ...
        while True:
            ...
            for variant in connection.get("nodes") or []:
                ...
                level_connection = item.get("inventoryLevels") or {}
                level_nodes = list(level_connection.get("nodes") or [])
                incoming, level_locations = _incoming_from_levels(level_nodes)
                locations.update(level_locations)

                page_info = level_connection.get("pageInfo") or {}
                level_after = page_info.get("endCursor")
                while page_info.get("hasNextPage") and item.get("id"):
                    inventory_followups += 1
                    more = client.graphql(
                        inventory_more_query,
                        {"id": item["id"], "after": level_after},
                    )
                    more_connection = ((more.get("inventoryItem") or {}).get("inventoryLevels") or {})
                    more_incoming, more_locations = _incoming_from_levels(
                        list(more_connection.get("nodes") or [])
                    )
                    incoming += more_incoming
                    locations.update(more_locations)
                    page_info = more_connection.get("pageInfo") or {}
                    level_after = page_info.get("endCursor")
    ...
    if not inventory.empty and "incoming_qty" in inventory.columns:
    ...
    if inventory.empty:
        return (
            ...
            empty_sales,
            {
                ...
                "inventory_method": "batched productVariants query",
            },
        )
    ...
    meta = {
        ...
        "locations": sorted(locations),
        "inventory_method": "batched productVariants; incoming temporarily forced to 0",
